...
p.ord <- plot_ordination(expt.ord, ord, type = "sites", color = "zone_code", shape="zone_code") + geom_point(alpha=0.7,aes(fill=zc_depth)) + theme(aspect.ratio=1) + geom_text(aes(label = season),show.legend = FALSE,size=1,vjust="bottom", nudge_y = -0.1)
...
annotation_cover <- data.frame(
   x = c(-4.2,4.2),
   y = c(45,45),
   label = c("More abundant in canopy areas\n(these compared by\ndepth in fig 5B)", "More abundant in grass inter-spaces\n(these compared by\ndepth in fig 5A)")
)
...
pdf(file = "figures/Figure 3 [fungal community correspondence analysis].pdf", width=5, height=5)
fig3
dev.off()
